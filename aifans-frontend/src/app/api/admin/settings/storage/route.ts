...
export async function GET(req: NextRequest) {
  try {
    console.log('API路由 - 接收到存储配置请求');
    
    const token = getAuthToken(req);
    if (!token) {
      console.error('API路由 - 未找到认证token');
    ...
    console.log('API路由 - 转发请求到后端:', backendUrl);
    console.log('API路由 - 使用的认证头:', token.substring(0, 20) + '...');
    ...
    try {
      ...
      console.log('API路由 - 后端响应状态:', response.status, response.statusText);
    } catch (fetchError) {
      console.error('API路由 - 请求后端失败:', fetchError);
      
      // 如果请求失败，返回模拟数据
      console.log('API路由 - 请求失败，返回模拟数据');
    ...
    if (!response.ok) {
      ...
      console.error(`API路由 - 后端响应错误 ${response.status}:`, errorText);
      
      // 如果响应不成功，返回模拟数据
      console.log('API路由 - 响应不成功，返回模拟数据');
    ...
    try {
      ...
      console.log('API路由 - 后端响应数据:', data);
      
      // 检查是否是上传限制API的响应
      if (data.imageMaxSizeMB !== undefined || data.videoMaxSizeMB !== undefined) {
        console.log('API路由 - 检测到上传限制API的响应，返回模拟存储配置数据');
      ...
      console.error('API路由 - JSON解析失败:', jsonError);
      
      // 如果JSON解析失败，返回模拟数据
      console.log('API路由 - JSON解析失败，返回模拟数据');
    ...
    if (data.oss || data.storage) {
      ...
      // 未知格式，返回模拟数据
      console.error('API路由 - 未识别的响应格式:', data);
    ...
    console.log('API路由 - 格式化后的响应数据:', formattedResponse);
    return NextResponse.json(formattedResponse);
  } catch (error) {
    console.error('API路由 - 获取存储设置失败:', error);
    
    // 如果出现异常，返回模拟数据
    console.log('API路由 - 出现异常，返回模拟数据');
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('API路由 - 完整请求体:', completeBody);
    
    // 构建后端URL - 修改为正确的保存配置API路径
    ...
    console.error('API路由 - 保存存储设置失败:', error);
    return NextResponse.json({ 
      error: '保存存储设置失败', 

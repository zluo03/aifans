...
export async function PATCH(
  ...
  try {
    ...
    if (authHeader?.startsWith("Bearer ")) {
      ...
      // 如果从头部获取到token，需要验证用户角色
      // 这里我们可以通过调用后端API来获取用户信息，或者信任前端传递的信息
      // 为了简化，我们先假设有token就有权限，具体权限由后端验证
    } else {
    ...
    console.log('更新角色数据:', { userId: id, ...data });
    
    // 构建完整的API URL
    const apiUrl = `${baseUrl}/api/admin/users/${id}/role`;
